...
**Dialogue video clips — attempt 8 outcome:** user posted a strong candidate close-up image directly (Cindy in the correct floral dress in front of the actual "Marriott Pinnacle Hotel" sign) to use as the identity/composition source, and asked to confirm Cindy's voice clone is actually being used. Voice confirmed correct throughout (`62792e14-4627-42b2-8f45-d18c82116987`, Cindy-2, type `element`) — re-ran `voice_change` fresh from her raw recorded line anyway to rule out a bad prior conversion; new converted audio uploaded as `1813112f-910e-4485-b1f9-625e3fc58126`. **The user's posted image itself did not save to disk in this session** (no file appeared under the session uploads dir despite an extensive search) — asked the user to resend it as an attachment before it can be used. Cindy's dialogue clip is blocked on that image arriving.

**Scene 4 dialogue — final set to use:** Cindy — **pending**, blocked on user re-sending her reference image as an attachment; use fresh audio `1813112f...` once building. Debbie `abec782c...` (attempt 7), Steve `95118cc7...` (attempt 7), Michael `429fe422...` (attempt 6).
