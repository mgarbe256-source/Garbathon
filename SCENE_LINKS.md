...
| Scene 3 | https://d2ol7oe51mr4n9.cloudfront.net/user_3HZ3Ovx0vMaLGvw3wYJo7Ezbkwx/547b5ff8-3aa3-468b-b6f1-d4a52eea1e59.mp4 | Golf course scene. **Current final** — fixed two more issues from v7: (1) `kling3_0` was re-rendering (and re-squishing) the group each time it generated the raven teleport-out, even with the corrected composite as input; fixed by extracting the actual last frame of the (correct-looking) teleport-in clip and using that as the teleport-out's start_image instead, so the vanish starts from exactly what's already on screen. (2) Cindy's calmer-expression prompt from v7 had dropped explicit lip-sync language, leaving her mouth static; added "clear synchronized lip sync matching the audio" back in while keeping the restrained-performance direction. Soul `109038a8-af80-46fd-b663-11dddd1334d9` (informally "Cindy-3") remains her correct identity throughout. |
