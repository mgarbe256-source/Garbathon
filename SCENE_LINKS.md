...
| Scene 3 | https://d2ol7oe51mr4n9.cloudfront.net/user_3HZ3Ovx0vMaLGvw3wYJo7Ezbkwx/c8ff77e7-be13-4ca2-bc31-7ff5e3d7c8d2.mp4 | Golf course scene. **Current final** — user flagged the teleport clips looked "fake" with no natural motion (people not glancing at each other or shifting, unlike Scenes 1-2). Root cause: v8's frozen-frame trick for the teleport-out (see below) fixed squishing but killed all motion by anchoring `kling3_0` to a single static image. Reverted to using the actual corrected composite as start/end image for both teleport clips, and enriched both prompts with natural motion language (glancing at each other, subtle breathing, weight shifts) plus explicit "keep proportions exactly as shown, do not stretch/distort" instructions to try to hold the squish fix while restoring motion. **Not yet visually confirmed** — watch for squish returning. Soul `109038a8-af80-46fd-b663-11dddd1334d9` (informally "Cindy-3") remains her correct identity throughout. |
...
9. `c8ff77e7-be13-4ca2-bc31-7ff5e3d7c8d2` — **current final** — user noted the teleports looked "fake," no movement, unlike Scenes 1-2 which had people glancing at each other and shifting naturally. Diagnosed two causes: (a) v8's frozen-frame anchor for teleport-out removed the model's freedom to add motion, (b) golf course prompts were flatter than Scene 1/2's (which explicitly requested "looking around... subtle natural breathing motion"). Fix: went back to feeding the actual corrected composite image (not a frozen frame) as start/end image for both teleport clips, and added explicit natural-motion language (glancing at each other, breathing, weight shifts) plus "keep proportions exactly as shown, do not stretch/distort" to try to prevent the squish from returning. This is a real trade-off attempt — needs visual confirmation from the user on both motion and squish.
