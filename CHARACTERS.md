...
**Richard cloned (2026-08-16):** user provided a fresh solo recording
(`Richard.m4a`, 32.5s, no long silences). Converted to mp3 locally and sent
back for the user to upload through the Higgsfield app (same flow as Ilana's).
The app's uploader auto-trimmed it to 15s (its apparent upload cap for this
flow) — confirmed media `b3dbd8be-eb05-4399-8549-c74d99d2b8a6` (15.0s wav).
Cloned via `create_voice_from_confirmed_audio` — **Richard** (`84f557fc-336e-467f-8eb2-0eace3d5833d`,
named "Richard-2" internally by Higgsfield), `completed`/`is_audio_eligible`.
**Use this voice_id for all Richard dialogue going forward** — supersedes the
Archie preset voice used as a placeholder until now.

All 3 voice-clone slots are now in use: Steve, Ilana, Richard.
...
| **Richard** | **Richard (clone)** | **`84f557fc-336e-467f-8eb2-0eace3d5833d`** | **element** |
